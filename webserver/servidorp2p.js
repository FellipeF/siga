...
const { MIN_APPROVALS } = require("./resources/blockchain/config");
...
class P2pserver {
    ...
    modificarBloco(index, newAuthorId) {

        if (index >= 0 && index < this.blockchain.chain.length) {
            // Encontra o bloco a ser modificado
            let block = this.blockchain.chain[index];
    
            // Vamos assumir que estamos modificando a primeira transação no bloco
            if (block.data.length > 0) {
                // Copia a transação existente
                let transaction = JSON.parse(JSON.stringify(block.data[0]));
    
                // Modifica o idAutor da transação
                transaction.input.data.idAutor = newAuthorId;
    
                // Cria a mensagem de transação
                const message = {
                    type: MESSAGE_TYPE.transaction,
                    transaction: transaction
                };
    
                // Propaga a transação modificada
                this.sockets.forEach(socket => {
                    socket.send(JSON.stringify(message));
                });
    
                console.log("Transação modificada e propagada:", transaction);
            } else {
                console.log("Nenhuma transação encontrada no bloco");
            }
        } else {
            console.log("Índice de bloco inválido");
        }
    }
